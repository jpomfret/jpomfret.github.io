...
components:
  ...
  changelog-hero:
    backgroundImage: "/img/hero-frame-{light,dark}.png"
...
The hero is the system's one deliberately committed, theme-independent moment: it renders on the fixed Brand Kit octopus/wave frame card regardless of whether the visitor is in light or dark mode, then sits directly above the ordinary token-driven feed. Everything past the hero is flat, quiet, and legible; the saturation is spent entirely on the hero so it doesn't leak into the rest of the page.

**Key Characteristics:**
- Full-palette role assignment: purple leads in light mode, teal leads in dark mode, navy and light-blue are the two grounds.
- One fixed frame-card hero per page, everything else theme-responsive.
...
- The hero's Brand Kit octopus/wave frame card is the system's single piece of illustrative artwork; the feed below it is undivided.
...
### Primary
- **Octopus Purple** (`#530080`): the light-mode acting accent — link/hover color, dated tag pill background, and the octopus mark's ink in the hero frame artwork. Has a deeper hover/active state, **Purple Deep** (`#38005a`).
...
### Tertiary
- **Brand Navy** (`#10265d`): the dark-mode page ground, and the hero's on-panel title/text and category-pill fill inside the frame card (navy reads dark-on-pale over the frame's pale center in both themes).
- **Brand Light-Blue** (`#bdecfe`): the dark-mode body-text tint and the category-pill text color inside the hero (which needs pale-panel-safe colors independent of the light/dark card tokens).
...
### Named Rules
**The One Saturated Moment Rule.** Only the hero renders on the fixed Brand Kit frame card — a per-theme octopus/wave PNG with a pale center panel (white in light, sky-blue in dark) that keeps the hero title dark-on-pale in both themes. It is fixed regardless of site theme; every other surface (cards, feed, sidebar) uses the light/dark token pairs.
...
- **Category pill (`changelog-categories a`):** low-opacity teal fill at rest (`rgba(79,170,179,0.15)` light / `0.22` dark) with a darker teal-tinted text color, brightening to a stronger teal opacity on hover (`0.28` light / `0.34` dark) over a 0.3s color transition. Inside the hero, pills switch to solid light-blue-on-navy at rest and solid teal-on-ink on hover, since the hero's fixed frame card can't use the theme-dependent low-opacity tokens.

### Cards / Containers
- **Corner Style:** 14px radius, matching the hero and hero image.
- **Background:** solid white (light) / navy-tinted `#17306a` (dark) for entries; the fixed Brand Kit frame-card image (`/img/hero-frame-{light,dark}.png`) for the hero, regardless of theme.
...
- **Do** keep the hero on the fixed Brand Kit frame-card image in both themes; it is the system's one deliberately saturated surface.
...
### Don't:
- **Don't** swap the hero's fixed frame card back to a per-post cover crop — that was tried and abandoned because posts' cover images carry baked-in title text that collides with the hero's own dynamic title.
